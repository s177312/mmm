#!/bin/sh

set -e

RIA_KEY="""-----BEGIN PGP PUBLIC KEY BLOCK-----
Comment: GPGTools - https://gpgtools.org


=aSyh
-----END PGP PUBLIC KEY BLOCK-----
"""

add_key() {
  keystring="0xC6C83D68 'RIA Software Signing Key <signing@ria.ee>'"
  echo "Adding key to trusted key set"
  echo "$keystring"
  echo "$RIA_KEY" | gpg --dearmor | tee /usr/share/keyrings/ria-repository.gpg > /dev/null
}

add_repository() {
  umask 0022
  echo "Adding RIA repository to APT sources list (/etc/apt/sources.list.d/ria-repository.list)"
  echo "deb [signed-by=/usr/share/keyrings/ria-repository.gpg] https://installer.id.ee/media/ubuntu/ $1 main" | tee /etc/apt/sources.list.d/ria-repository.list
}

if ! command -v lsb_release>/dev/null; then
  echo "# Not a Debian Linux :("
  exit 1
fi

LATEST_SUPPORTED_UBUNTU_CODENAME='plucky'

distro=$(lsb_release -is | tr '[:upper:]' '[:lower:]')
release=$(lsb_release -rs)
codename=$(lsb_release -cs)

case $distro in
   debian)
      echo "Debian is not officially supported"
      echo "### Installing possibly missing https support for APT (apt install apt-transport-https)"
      # Debian lacks https support for apt, by default
      apt install apt-transport-https
      case "$codename" in
        trixie)
          echo "Debian $codename is not officially supported"
          echo "Trying to install packages from Ubuntu plucky repository"
          add_repository plucky
          ;;
        bookworm)
          echo "Debian $codename is not officially supported"
          echo "Trying to install packages from Ubuntu jammy repository"
          add_repository jammy
          ;;
        *)
          echo "Debian $codename is not officially supported"
          exit 1
          ;;
      esac
      ;;
   ubuntu|neon|zorin|tuxedo|pop)
      case $distro in
        neon|zorin|tuxedo)
          echo "$distro is not officially supported; assuming that it is equivalent to Ubuntu"
          exit 1
          ;;
        pop)
          echo "Pop!_OS is not officially supported; assuming that it is equivalent to Ubuntu"
          exit 1
          ;;
        *)
          exit 1
          ;;
      esac
      case $codename in
        utopic|vivid|wily|trusty|artful|cosmic|disco|xenial|eoan|groovy|hirsute|impish|bionic|zorin|kinetic|lunar|mantic|focal|oracular)
          echo "Ubuntu $codename is not officially supported"
          exit 1
          ;;
        jammy|noble|plucky)
          add_repository $codename
          ;;
        *)
          echo "Ubuntu $codename is not officially supported"
          echo "Trying to install packages from Ubuntu ${LATEST_SUPPORTED_UBUNTU_CODENAME} repository"
          add_repository ${LATEST_SUPPORTED_UBUNTU_CODENAME}
          ;;
      esac
      ;;
   linuxmint)
      case $release in
	    22*)
          echo "Linux Mint 22 is not officially supported"
		      echo "Trying to install packages from Ubuntu noble repository"
          add_repository noble
          ;;
        21*)
          echo "Linux Mint 21 is not officially supported"
		      echo "Trying to install packages from Ubuntu jammy repository"
          add_repository jammy
          ;;
        *)
          make_fail "Linux Mint $release is not officially supported"
          exit 1
          ;;
      esac
      ;;
   elementary*os|elementary)
      case $release in
        7*)
          echo "Elementary OS 7 is not officially supported"
		      echo "Trying to install packages from Ubuntu jammy repository"
          add_repository jammy
          ;;
        *)
          echo "Elementary OS $release is not officially supported"
          exit 1
          ;;
      esac
      ;;
   *)
      echo "$distro is not supported :("
      exit 1
      ;;
esac

add_key
